...
export function getG1Pubkey(secret: SecretKey): PublicKey {
  const pubkey: any = mcl.mul(g1(), secret);
  pubkey.normalize();
  return pubkey;
}
...
export function signSimpleMessage(message: any, secretKey: mcl.Fr): Signature {
  const DOMAIN = ethers.utils.arrayify(ethers.utils.solidityKeccak256(["string"], ["YOUR_DOMAIN_STRING"]));
  const messagePoint = hashToPoint(message, DOMAIN);
  const signature: any = mcl.mul(messagePoint, secretKey);
  signature.normalize();
  console.log(verifyRaw(signature, messagePoint, getPubkey(secretKey)));
...
export function signChainMessage(
  domain: Domain,
  chainId: number,
...
export function hashAndMapToG2(msg: string): mclG2 {
  return mcl.hashAndMapToG2(msg);
}
...
export function signMessagePoint(
...
export function verifyRaw(signature: Signature, pubkey: PublicKey, message: MessagePoint): boolean {
  ...
  if (!(signature instanceof mcl.G1)) {
    throw new Error("signature is not instance of G1")

  }
  if (!(message instanceof mcl.G1)) {
    throw new Error("message is not instance of G1")

  }
  if (!(negG2 instanceof mcl.PrecomputedG2)) {
    throw new Error("negG2 is not instance of PrecomputedG2")
  }
  if (!(pubkey instanceof mcl.G2)) {
    throw new Error("pubkey is not instance of G2")
  }
...
export function hashAndMapToG1(msg: string): mclG1 {
  return mcl.hashAndMapToG1(msg);
}

export class BlsVerifier {
  constructor() { }
  public verify(signature: solG1, pubkey: solG2, messagePoint: MessagePoint) {
...
export class BlsSigner {
  ...
  get pubkeySol(): solG2 {
    // swap x0 and x1 to match the Solidity representation  convention used in the bn254 pairing precompile contract
    ///      "Elements a * i + b of F_p^2 are encoded as two elements of F_p, (a, b)."
    ///      https://github.com/ethereum/EIPs/blob/master/EIPS/eip-197.md#encoding
    const original = g2ToHex(this._pubkey);
    return [original[1], original[0], original[3], original[2]];


  }

  public sign(message: any): SignatureInterface {
    const g1string = message[0]._hex + message[1]._hex.slice(2);
    const g1 = parseG1(loadG1(g1string));
    const { signature } = signMessagePoint(g1, this.secret);
    const sol = g1ToHex(signature);
    return { mcl: signature, sol, messagePoint: g1 };
  }
}
